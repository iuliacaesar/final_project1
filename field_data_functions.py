...
def add_data(what_you_build, building_data, obj):
    ...
    if what_you_build == 'house':
        for i in range(HOUSE_HIGHT//len_height):
            for j in range(HOUSE_LEN//len_width):
                building_data[obj.y//len_height + i][obj.x//len_width + j] = 1
    if what_you_build == 'electricity':
        for i in range(ELECTRICITY_HIGHT//len_height):
            for j in range(ELECTRICITY_LEN//len_width ):
                building_data[obj.y//len_height + i][obj.x//len_width + j] = 1
    if what_you_build == 'water':
        for i in range(WATER_HIGHT//len_height):
            for j in range(WATER_LEN//len_width):
                building_data[obj.y//len_height + i][obj.x//len_width + j] = 1
